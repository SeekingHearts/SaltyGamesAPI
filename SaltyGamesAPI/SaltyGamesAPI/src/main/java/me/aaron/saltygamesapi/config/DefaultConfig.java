...
public class DefaultConfig {
	...
	public DefaultConfig(final JavaPlugin pl, final boolean custom) {
		this.pl = pl;
		// DefaultConfig.
	}

	public static void init(final JavaPlugin pl, final boolean custom) {
		final FileConfiguration config = pl.getConfig();

		config.options()
				.header("Dies ist die Standart-Config. \n"
						+ "Du findest die folgenden Einträge in den in []-Klammern folgenden Dateien! \n"
						+ "Klassen (= Kits) [classes.yml] \n" + "Arenen (alle gespeicherten) [arenas.yml] \n"
						+ "Nachrichten [messages.yml] \n"
						+ "Du kannst Achievements (Errungenschaften) in [achievements.yml] de-/aktivieren.");
		...
		if (!custom) {
			...
			config.addDefault(ArenaConfigStrings.CONFIG_USE_CREDITS_INSTEAD_MONEY_FOR_KITS, false);
			...
			config.addDefault(ArenaConfigStrings.CONFIG_REWARDS_ECONOMY_REWARD_FOR_PARTICIPATION, false);
            ...
            config.addDefault(ArenaConfigStrings.CONFIG_CMDS_AFTER, "say SERVER WIRD GESTOPPT;stop");
            ...
            config.addDefault(ArenaConfigStrings.CONFIG_EXTRA_LOBBY_ITEM_PREFIX + "item0" + ArenaConfigStrings.CONFIG_EXTRA_LOBBY_ITEM_COMMAND_SUFFIX, "say Dies ist ein Custom Extra Lobby Item.");
            ...
            config.addDefault(ArenaConfigStrings.CONFIG_MYSQL_DATABASE, "saltyminigames");
		...
		try {
			DefaultConfig.convert(pl);
		} catch (final Exception e) {
			;
		}
	}
	
	public static void convert(final JavaPlugin pl) throws IOException {
		final FileConfiguration config = pl.getConfig();
