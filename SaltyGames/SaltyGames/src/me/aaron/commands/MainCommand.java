...
public class MainCommand implements CommandExecutor {
	...
	public MainCommand(SaltyGames plugin){
		this.plugin = plugin;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(!sender.hasPermission(Permission.USE.getPermissions())){
		...
		if(args.length == 1){
			...
			for(String id : subCommands.keySet()){
				...
				if(subCommands.get(id).contains(args[0].toLowerCase())){
					...
					if(!sender.hasPermission(Permission.OPEN_ALL_GAME_GUI.getPermissions()) && !sender.hasPermission(Permission.OPEN_GAME_GUI.getPermission(id))){
			...
			if(args[0].equalsIgnoreCase("help") || args[0].equals("?")){
				if(!sender.hasPermission(Permission.CMD_HELP.getPermissions())){
					sender.sendMessage(lang.PREFIX + lang.CMD_NO_PERM);
					return true;
				}
			...
			if(args[0].equalsIgnoreCase("info")){
				if(!sender.hasPermission(Permission.CMD_INFO.getPermissions())){
					sender.sendMessage(lang.PREFIX + lang.CMD_NO_PERM);
					return true;
				}
				...
				for(String gameID : subCommands.keySet()) {
					...
					for (String message : lang.CMD_INFO_PER_GAME) {
						sender.sendMessage(lang.PREFIX + message
								...
								.replace("%playerNum%", String.valueOf(game.getGameSettings().getGameType().getPlayerNumber())));
					}
				}
				// send info footer
