...
import main.me.aaron.utils.IconMenu;
...
public class Guns {
	...
	public void openGUI(final String p) {
		final int credits = MinigamesAPI.getAPI().getPluginInstance(pl).getStatsInstance().getPoints(p);
		IconMenu iconm;
		if (this.lastmainiconm.containsKey(p)) {
			...
			iconm = new IconMenu("Gun Upgrades (Credits: " + credits + ")", 36, event -> {
				...
				if (MinigamesAPI.getAPI().getPluginInstance(pl).getAllGuns().containsKey(d)) {
					...
					final Gun g = MinigamesAPI.getAPI().getPluginInstance(pl).getAllGuns().get(gun);
					if (g != null) {
						final int[] pattributes = Guns.this.getPlayerGunAttributeLevels(pl, p1.getName(), g);
						HashMap<Gun, int[]> t;
						...
						if (d.startsWith("Speed")) {
							...
							cost = Math.pow(level_multiplier, i1) * Guns.this.speed_cost;
							if (i1 < 3 && credits >= cost) {
								Guns.this.openUpgradeGUI(p1.getName(), gun, "speed", pattributes[0] + 1, cost);
								done = true;
								// setPlayerGunLevel(plugin, p.getName(), gun,
								// "speed", pattributes[0] + 1);
							}
						} else if (d.startsWith("Durability")) {
							final int i2 = pattributes[1];
							cost = Math.pow(level_multiplier, i2) * Guns.this.durability_cost;
							...
							cost = Math.pow(level_multiplier, i3) * Guns.this.shoot_amount_cost;
							...
							cost = Math.pow(level_multiplier, i4) * Guns.this.knockback_multiplier_cost;
						...
						if (!done) {
							p1.sendMessage(
									MinigamesAPI.getAPI().getPluginInstance(pl).getMessagesConfig().not_enough_credits
											.replaceAll("<credits>", Double.toString(cost)));
						}
					}
				}
		...
		int c = 0;
		for (final String ac : MinigamesAPI.getAPI().getPluginInstance(pl).getAllGuns().keySet()) {
			final Gun ac_ = MinigamesAPI.getAPI().getPluginInstance(pl).getAllGuns().get(ac);
			final int[] pattributes = this.getPlayerGunAttributeLevels(pl, p, ac_);
			iconm.setOption(c, ac_.icon.get(0), ac, MinigamesAPI.getAPI().getPluginInstance(pl).getGunsConfig()
					.getConfig().getString("config.guns." + ac + ".lore"));
			iconm.setOption(c + 2, new ItemStack(Material.SUGAR), "Speed Lv " + ChatColor.DARK_RED + pattributes[0],
					ac + " Speed Upgrade");
			iconm.setOption(c + 3, new ItemStack(Material.DIAMOND),
					"Durability Lv " + ChatColor.DARK_RED + pattributes[1], ac + " Durability Upgrade");
			iconm.setOption(c + 4, new ItemStack(Material.EGG), "Shoot Lv " + ChatColor.DARK_RED + pattributes[2],
					ac + " Shoot amount Upgrade");
			iconm.setOption(c + 5, new ItemStack(Material.STICK), "Knockback Lv " + ChatColor.DARK_RED + pattributes[3],
					ac + " Knockback Upgrade");
			c += 9;
		}
	...
	public void setPlayerGunLevel(final JavaPlugin plugin, final String p, final String g, final String attribute,
			final int level, final double cost) {
	...
	public void setPlayerGunMain(final JavaPlugin plugin, final String p, final String g, final boolean val) {
		...
		if (this.getPlayerAllMainGunsCount(plugin, p) > 1 && val) {
			Bukkit.getPlayer(p).sendMessage(
					MinigamesAPI.getAPI().getPluginInstance(plugin).getMessagesConfig().too_many_main_guns);
			return;
		}
		config.set(path, val);
		MinigamesAPI.getAPI().getPluginInstance(plugin).getGunsConfig().saveConfig();
	...
	public void openGunMainEditGUI(final String p, final String g) {
		...
		p_.sendMessage(
				MinigamesAPI.getAPI().getPluginInstance(pl).getMessagesConfig().all_guns.replaceAll("<guns>", guns));
		...
		if (this.lastmainediticonm.containsKey(p)) {
			...
			iconm = new IconMenu("Set Main Gun", 9, event -> {
				...
				final Player p1 = event.getPlayer();
				if (d.startsWith("Set")) {
					Guns.this.setPlayerGunMain(pl, p1.getName(), g, true);
				} else if (d.startsWith("Remove")) {
					Guns.this.setPlayerGunMain(pl, p1.getName(), g, false);
				}
	...
	public void openUpgradeGUI(final String p, final String g, final String attribute, final int level,
			final double cost) {
		IconMenu iconm;
		if (this.lastupgradeiconm.containsKey(p)) {
			...
			iconm = new IconMenu("Upgrade?", 9, event -> {
				...
				final Player p1 = event.getPlayer();
				if (d.startsWith("Buy")) {
					Guns.this.setPlayerGunLevel(pl, p1.getName(), g, attribute, level, cost);
					p1.sendMessage(
							MinigamesAPI.getAPI().getPluginInstance(pl).getMessagesConfig().attributelevel_increased
	...
	public static void loadGuns(final JavaPlugin plugin) {
		...
		if (config.isSet("config.guns")) {
			for (final String gun : config.getConfigurationSection("config.guns.").getKeys(false)) {
				...
				final Gun n = new Gun(plugin, gun, config.getDouble(path + "speed"), config.getInt(path + "durability"),
						config.getInt(path + "shoot_amount"), config.getDouble(path + "knockback_multiplier"),
						Egg.class, Util.parseItems(config.getString(path + "items")),
						Util.parseItems(config.getString(path + "icon")));
	...
	public ArrayList<String> getAllMainGuns(final Player p) {
		final FileConfiguration config = MinigamesAPI.getAPI().getPluginInstance(pl).getGunsConfig().getConfig();
	...
	public void giveMainGuns(final Player p) {
		...
		if (config.isSet("players." + p.getName() + ".")) {
			...
			if (count < 1) {
				...
				final HashMap<String, Gun> guns = MinigamesAPI.getAPI().getPluginInstance(pl).getAllGuns();
